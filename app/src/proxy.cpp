...
void await_signals(std::unique_ptr<sdbus::IProxy> &proxy,
                   std::shared_ptr<sch_printer::Printer> &printer)
{
    ...
    proxy->uponSignal(SIGNAL_NAMES::changed)
        ...
        .call(
            [printer](const Config &conf)
            {
                ...
                {
                    std::cerr
                        << "Cannot extract field from the conf: " << e.what()
                        << "\n";
                    exit(EXIT_FAILURE);
                }
    ...
    // uponSignal as registration (creating subscriber),
    // finishRegistration() is necessary
    proxy->finishRegistration();
}

void read_conf(std::unique_ptr<sdbus::IProxy> &proxy,
               std::shared_ptr<sch_printer::Printer> &printer)
{
    ...
    try
    {
        ...
        if (debug_mode)
            std::cout << "Received conf\n";
    }
    catch (const sdbus::Error &e)
    {
        std::cerr << "Error while calling a method to server: " << e.what()
                  << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    if (debug_mode)
        std::cout << "Configuration was extracted and read: " << phrase << "\n";
